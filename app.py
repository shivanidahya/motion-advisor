  ...
  <main class="shell">
    ...
    <div class="workspace">
      <section class="form-panel">
        <div class="eyebrow">Step 01 / Make the choice</div><h2>Should AI help draft this motion?</h2><p class="sub">Start with the motion type, then weigh the information, stakes, and review available. Your responses shape a workflow recommendation, not a legal opinion.</p>
        <form id="advisorForm">
          <div class="field"><label>What kind of motion is this?</label><div class="options"><label class="option"><input type="radio" name="type" value="routine" required> Routine or procedural</label><label class="option"><input type="radio" name="type" value="dispositive"> Dispositive (could resolve claims or the case)</label><label class="option"><input type="radio" name="type" value="evidentiary"> Evidentiary or discovery</label><label class="option"><input type="radio" name="type" value="emergency"> Emergency or time-sensitive</label><label class="option"><input type="radio" name="type" value="substantive"> Other substantive motion</label></div><div class="hint">Common examples include motions to dismiss, summary judgment, compel discovery, exclude evidence, for a protective order, or for a temporary restraining order.</div></div>
          ...
          <div class="field"><label>What would you ask AI to do?</label><div class="options"><label class="option"><input type="radio" name="task" value="bounded" required> Organize notes, outline, or improve plain-language phrasing</label><label class="option"><input type="radio" name="task" value="draft"> Generate substantial filing-ready argument</label></div><div class="hint">The narrower the task, the easier it is to verify and supervise.</div></div>
      ...
      <aside class="aside"><h3>What we’re weighing</h3><p>The choice is not simply “AI or no AI.” It is whether this task can be bounded, supervised, and checked without compromising the client or the filing.</p><div class="signal"><span class="dot"></span><span><b>Motion type and consequence</b><br>Procedural requests are different from dispositive or emergency relief.</span></div><div class="signal"><span class="dot"></span><span><b>Confidentiality</b><br>Can the information safely enter your approved tool?</span></div><div class="signal"><span class="dot"></span><span><b>Accuracy and authority</b><br>What is the cost of an unsupported fact, quote, or citation?</span></div><div class="signal"><span class="dot green"></span><span><b>Oversight and time</b><br>Who will verify the output before filing?</span></div><div class="signal"><span class="dot green"></span><span><b>Task scope</b><br>Can AI assist with structure without deciding strategy?</span></div></aside>
    </div>
  </main>
  <script>
    ...
    document.getElementById('advisorForm').addEventListener('submit', e => { e.preventDefault(); const data = new FormData(e.target); const highRisk = data.get('sensitive') === 'yes' || data.get('review') !== 'full' || ['dispositive', 'emergency', 'substantive'].includes(data.get('type')) || data.get('task') === 'draft'; const box = document.getElementById('resultBox'); document.getElementById('resultTitle').textContent = highRisk ? 'Keep the legal judgment human-led.' : 'Use AI as a supervised drafting assistant.'; document.getElementById('resultText').textContent = highRisk ? 'These factors make full AI-generated drafting a poor fit. You may still use an approved tool for bounded support, but keep strategy, factual assertions, authorities, and final language under direct attorney control.' : 'This looks like a reasonable candidate for carefully supervised AI assistance with structure and wording. Verify every source, quotation, factual assertion, and filing requirement before relying on the result.'; document.getElementById('goodUse').textContent = highRisk ? 'Build a neutral outline from attorney notes, create a checklist, or flag issues for human review.' : 'Organize attorney-provided facts, suggest neutral phrasing, summarize authorities you have checked, and identify repetitive sections.'; document.getElementById('humanUse').textContent = highRisk ? 'Legal strategy, dispositive reasoning, emergency relief, restricted information, citations, and all filing-ready language.' : 'Final argument, choice of authorities, factual record, privilege decisions, and filing approval.'; box.classList.toggle('caution', highRisk); document.getElementById('result').classList.add('visible'); document.getElementById('result').scrollIntoView({behavior:'smooth', block:'nearest'}); });
  </script>
</body></html>'''
